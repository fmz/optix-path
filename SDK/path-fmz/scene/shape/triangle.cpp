...
static float calculateTriArea(const Vector3f& v1, const Vector3f& v2, const Vector3f& v3) {
    // Heron's formula (but numerically stable: see wikipedia)
    float a = (v2 - v1).norm();
    float b = (v3 - v1).norm();
    float c = (v3 - v2).norm();
...
{
    _area = calculateTriArea(v1,v2,v3);
